      ...
      <table class='table table-light table-sm table-bordered table-hover table-striped table-responsive-md' id="listUl">
        <thead class="thead-dark align-middle">
          <tr> 
            ...
            <th  class='align-middle' scope="col"> Last Visited Site </th> 
            <th  class='align-middle' scope="col"> Time </th> 
          </tr>
        </thead>

        <?php
            $query = "SELECT users.id, product_id, username, password, last_visited 
            FROM hyperkar.users INNER JOIN (SELECT user_id, product_id, MAX(time) AS last_visited 
            FROM hyperkar.visit_history GROUP BY user_id) latesttime ON users.id=latesttime.user_id";
